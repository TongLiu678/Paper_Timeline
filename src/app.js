...
const state = { query: "", field: "all", status: "all", view: "timeline", month: shanghaiDateKey(new Date()).slice(0, 7) };
...
function renderHero() {
  const next = getUpcomingMilestones(conferences)[0];
  const target = $("#hero-next");
  if (!next) {
    target.innerHTML = `<div class="next-empty">目前没有已公布的未来截稿节点</div>`;
    return;
  }
...
function render() {
  ...
  $("#result-label").textContent = `找到 ${filtered.length} 场会议`;
...
$("#timeline-view").addEventListener("click", (event) => { const button = event.target.closest("[data-export]"); if (!button) return; const conference = conferences.find((item) => item.id === button.dataset.export); if (conference) downloadIcs(getUpcomingMilestones([conference]), `${conference.acronym.toLowerCase()}-deadlines.ics`); });
$("#export-ics").addEventListener("click", () => { const filtered = filterConferences(conferences, state); downloadIcs(getUpcomingMilestones(filtered), "paper-timeline-deadlines.ics"); });
